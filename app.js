...
function refreshGate() {
  enterBtn.disabled = !(consentCheck.checked && displayName.value.trim().length >= 1);
}
...
function initMap() {
  state.map = L.map("map", { zoomControl: true, attributionControl: true }).setView([6.5244, 3.3792], 13);
...
function centerOnMeOnce() {
  if (centered || !state.myPos) return;
  centered = true;
...
function colorFor(uid) {
  let h = 0; for (const c of uid) h = (h * 31 + c.charCodeAt(0)) % 360;
  return `hsl(${h} 70% 55%)`;
}
function drawMarker(uid) {
  ...
  if (state.markers[uid]) {
    state.markers[uid].setLatLng([p.lat, p.lng]).setIcon(icon);
  } else {
    state.markers[uid] = L.marker([p.lat, p.lng], { icon }).addTo(state.map);
  }
}
...
function subscribeGroup() {
  ...
  mod.onValue(r, (snap) => {
    ...
    Object.keys(state.people).forEach(uid => {
      if (uid !== state.uid && !seen.has(uid) && !uid.startsWith("demo_")) removePerson(uid);
    });
  });
}
...
function wireUi() {
  ...
  $("joinGroup").addEventListener("click", () => {
    ...
    state.group = code;
    localStorage.setItem(LS.group, code);
    updateGroupPill();
...
function updateModeBadge() {
  const b = $("modeBadge");
  if (state.mode === "live") { b.textContent = "Live"; b.className = "badge live"; }
